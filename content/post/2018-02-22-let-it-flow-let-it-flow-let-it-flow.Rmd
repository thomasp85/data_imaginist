...
<img src="/assets/images/particles_logo_small.png" style="width:50%;max-width:200px;display:block;margin-left:auto;margin-right:auto"/>

I got a new package on CRAN! It's called `particles` and it sits right in the 
perfect intersection of fun, cool, and useful. If you don't believe me, have a
look at the Venn diagram below — charts don't lie!

![](/assets/images/particles_venn.png)

## So what is this all about?
`particles` is an R implementation of The `d3-force` algorithm developed by Mike 
Bostock and can be used to simulate many different types of interactions between 
particles and the world. While `particles` can be used as a simple physics 
engine it has not been developed with this in mind and accuracy, in terms of how 
well it behaves like the physical world, has not been a main priority during 
development.
...
force) and then we add a new manybody force that attracts instead (these two
steps could also have been done in one using `rewield()`). Then we heat
...
> Note that we are able to manipulate the underlying `tbl_graph` directly using
`active()`, `mutate()`, and `filter()` - `particles` defines a subset of the
verbs defined in `tidygraph`/`dplyr`
